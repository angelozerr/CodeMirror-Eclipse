...
*/
	...
	private Composite createButtonSection(Composite parent) {
		...
		fFindNextButton= makeButton(panel, "Fi&nd"/*EditorMessages.FindReplace_FindNextButton_label*/, 102, true, new SelectionAdapter() {
		...
		fReplaceFindButton= makeButton(panel, "Replace/Fin&d" /*EditorMessages.FindReplace_ReplaceFindButton_label*/, 103, false, new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
		...
		fReplaceSelectionButton= makeButton(panel, "&Replace"/*EditorMessages.FindReplace_ReplaceSelectionButton_label*/, 104, false, new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				if (fNeedsInitialFindBeforeReplace)
					performSearch();
		...
		fReplaceAllButton= makeButton(panel, "Replace &All"/*EditorMessages.FindReplace_ReplaceAllButton_label*/, 105, false, new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				performReplaceAll();
				updateFindAndReplaceHistory();
			}
	...
	//	group.setText(EditorMessages.FindReplace_Options);
		...
		fWithOverlayCheckBox = new Button(group, SWT.CHECK | SWT.LEFT);
		fWithOverlayCheckBox.setText("Overlay");
		setGridData(fWithOverlayCheckBox, SWT.LEFT, false, SWT.CENTER, false);
		fWithOverlayCheckBox.setSelection(fWithOverlayInit);
		fWithOverlayCheckBox.addSelectionListener(selectionListener);
		storeButtonWithMnemonicInMap(fWithOverlayCheckBox);
	 ...
	 * @since 3.0
	 */
	private boolean findNext(String findString, boolean forwardSearch, boolean withOverlay, boolean caseSensitive, boolean wrapSearch, boolean wholeWord, boolean incremental, boolean regExSearch, boolean beep) {

		if (fTarget == null)
			return false;

		boolean result = fTarget.findNext(findString, forwardSearch, withOverlay);
		if (!result) {
...
*/
	...
	private void storeSettings() {
		...
		fWithOverlayInit = isWithOverlay();
	...
	private void initFindStringFromSelection() {
		if (fTarget != null && okToUse(fFindField)) {
			...
			if (fullSelection.length() > 0) {
				String firstLine= getFirstLine(fullSelection, fTarget.getLineSeparator());
				String pattern= firstLine;//isRegEx ? FindReplaceDocumentAdapter.escapeForRegExPattern(fullSelection) : firstLine;
...
*/
	...
	private void performSearch(boolean mustInitIncrementalBaseLocation, boolean beep, boolean forwardSearch) {
		...
		if (findString != null && findString.length() > 0) {
			...
			try {
				...
				somethingFound= findNext(findString, forwardSearch, isWithOverlay(), isCaseSensitiveSearch(), isWrapSearch(), isWholeWordSearch(), isIncrementalSearch() && !isRegExSearchAvailableAndChecked(), isRegExSearchAvailableAndChecked(), beep);
			} catch (PatternSyntaxException ex) {
				statusError(ex.getLocalizedMessage());
			} catch (IllegalStateException ex) {
				// we don't keep state in this dialog
	...
	private int replaceAll(String findString, String replaceString, boolean forwardSearch, boolean caseSensitive, boolean wholeWord, boolean regExSearch) {
		...
		if (fTarget != null) {
			return fTarget.replaceAll(findString, replaceString);
		}
		return 0;
		/*int replaceCount= 0;
	...
	private IDialogSettings getDialogSettings() {
		IDialogSettings settings= fSettings; //TextEditorPlugin.getDefault().getDialogSettings();
	...
	private void writeConfiguration() {
		IDialogSettings s= getDialogSettings();
		s.put("overlay", fWithOverlayInit); //$NON-NLS-1$
