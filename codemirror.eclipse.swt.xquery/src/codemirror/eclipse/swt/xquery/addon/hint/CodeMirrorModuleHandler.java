...
public class CodeMirrorModuleHandler implements ModuleHandler {
  ...
  public void startModule(String prefix, String namespaceURI, String location) throws Exception {
    startModule(prefix, null, namespaceURI, location);
  }

  public void startModule(String prefix, Boolean prefixRequired, String namespaceURI, String location) throws Exception {
    this.nbFunctions = 0;
    ...
    if (hasPrefix) {
      ...
      if (prefixRequired != null && !prefixRequired) {
        JsonUtils.addJsonField("prefixRequired", false, false, true, writer);
      }
    }
  ...
  public void addFunction(String funcName, List<Parameter> parameters, String returnType, String doc) throws Exception {
    if (nbFunctions > 0) {
    ...
    if (parameters != null) {
      ...
      for (Parameter parameter : parameters) {
        ...
        JsonUtils.beginJsonObject(writer);
        JsonUtils.addJsonField("name", parameter.getName(), true, true, writer);
